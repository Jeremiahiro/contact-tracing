...
! function (t) {
    t.fn.rescalendar = function (a) {
        ...
        function o(a, e) {
            for (var n = l.format, d = moment(e, n).subtract(l.jumpSize, "days"), s = (moment(e, n).add(l.jumpSize, "days"), moment().startOf("day")), i = '<td class="firstColumn"></td>', c = "", f = "", m = "", u = "", _ = 0, v = "", y = "", p = "", b = "", D = a.find("input.refDate").val(), h = 0; h < l.calSize + 1; h++) b = "", f = (c = moment(d).add(h, "days")).format(n), m = c.format("DD"), v = c.locale(l.locale).format("MMM").replace(".", ""), u = c.locale(l.locale).format("dd"), _ = c.day(), y = f == s.format(n) ? "today" : "", p = f == D ? "middleDay" : "", (l.disabledDays.indexOf(f) > -1 || l.disabledWeekDays.indexOf(_) > -1) && (b = "disabledDay"), i += ['<td class="day_cell ' + y + " " + p + " " + b + '" data-cellDate="' + f + '">', '<span class="m-0 regular f-12 dia_semana">' + u + "</span>", '<span class="f-24 bold m-0 dia">' + m + "</span>", '<span class="mes">' + v + "</span>", "</td>"].join("");
